...
const excelUploadForm = useForm({ file: null });
...
const handleExcelFileChange = (e) => {
    ...
    excelUploadForm.file = e.target.files[0];
};

const uploadExcel = () => {
    if (!excelFile.value) return;
    excelUploadForm.post(route('teams.add-users-excel', props.team), {
        forceFormData: true,
        onSuccess: () => {
            // closeExcelModal();
        },
...
<template>
    <div>
                ...
                <template #form>
                    <div class="col-span-6">
                        <div class="max-w-xl text-sm text-gray-600">
                            Please provide the email address of the person you would like to add to this team. o 
                            <div class="text-blue-500 cursor-pointer" @click="openExcelModal">añadelo desde un excel</div>
                        </div>
                    </div>
        ...
        <DialogModal :show="showExcelModal" @close="closeExcelModal">
            <template #title>
                Añadir miembros desde Excel
            </template>
            <template #content>
                <div v-if="page.props.flash.success" class="mb-2 text-green-600">{{ page.props.flash.success }}</div>
                // <div v-if="excelUploadForm.errors.file" class="mb-2 text-red-600">{{ excelUploadForm.errors.file }}</div>
                <input type="file" accept=".xlsx,.xls" @change="handleExcelFileChange" />
                <InputError :message="excelUploadForm.errors.file" class="mt-2" />
            </template>
            
            <template #footer>
                <SecondaryButton @click="closeExcelModal">Cancelar</SecondaryButton>
                <PrimaryButton class="ms-3" :class="{ 'opacity-25': excelUploadForm.processing }" :disabled="excelUploadForm.processing || !excelFile" @click="uploadExcel">
                    Subir
                </PrimaryButton>
            </template>
        </DialogModal>
